...
export default function CandidateAssessments({
    ...
    const checkStatus = async () => {
      ...
      try {
        const responseCheck = await fetch(
          `/api/assessment-responses/${selectedApplicationId}`
        );
        if (responseCheck.ok) {
        ...
        const assessmentCheck = await fetch(
          `/api/assessments/${selectedApp.jobId}`
        );
  ...
  const renderSelectorContent = () => {
    ...
    switch (assessmentStatus) {
      case "loading":
        button = (
          <div className="w-full mt-4 h-10 bg-slate-700 rounded-lg flex items-center justify-center">
      ...
      case "available":
        if (selectedApp && selectedApp.stage === "tech") {
          button = (
            <button
              onClick={() => setIsTakingAssessment(true)}
              className="w-full mt-4 bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700 transition-all shadow-lg hover:shadow-indigo-600/40 transform hover:-translate-y-px"
            >
              Take Assessment
            </button>
          );
          ...
          button = (
            <button
              disabled
              className="w-full mt-4 bg-slate-800 text-slate-500 font-bold py-2 px-4 rounded-lg cursor-not-allowed border border-slate-700"
            >
              Only available at Technical stage
            </button>
          );
        }
        break;
      ...
      case "submitted":
        button = (
          <div className="w-full mt-4 text-center bg-green-500/10 text-green-300 font-semibold py-2 px-4 rounded-lg flex items-center justify-center border border-green-500/30">
            <Check size={18} className="mr-2" /> Assessment Submitted
          </div>
        );
        break;
      case "unavailable":
    ...
    return (
      ...
      >
        ...
        <AnimatePresence>
          <motion.div
            key={assessmentStatus + selectedApplicationId} // Add key to force re-render on selection change
  ...
  return (
    <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6 shadow-lg">
      <h3 className="text-xl font-semibold text-white flex items-center">
        <BookOpen className="mr-3 text-indigo-400" />
        Assessments
      </h3>
